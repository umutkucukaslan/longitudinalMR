...
EXPERIMENT_NAME = 'exp_2020_04_27'
...
def train_discriminator_only_step(input_image, target):
...
def train_generator_only_step(input_image, target):
    with tf.GradientTape() as gen_tape:
        ...
        gen_output = generator(input_image, training=True)
        gen_l1_loss = generator_l1_loss(gen_output, target)
...
def eval_discriminator_only_step(input_image, target):
    gen_output = generator(input_image, training=False)
    ...
    for epoch in range(initial_epoch, num_epochs):
        ...
        for n, input_image in train_ds.enumerate():
            if TRAIN_ADVERSARIALLY:
                ...
                disc_loss = train_discriminator_only_step(input_image, input_image)
                losses[0].append(0)
                losses[1].append(0)
                losses[2].append(0)
                losses[3].append(disc_loss.numpy())
        ...
        for input_image in val_ds:
            if TRAIN_ADVERSARIALLY:
                ...
                disc_loss = eval_discriminator_only_step(input_image, input_image)
                val_losses[0].append(0)
                val_losses[1].append(0)

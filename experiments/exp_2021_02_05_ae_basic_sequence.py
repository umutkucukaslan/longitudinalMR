...
"""
basic autoencoder trained with long. sequences
...
if __name__ == "__main__":
    ...
    def get_predicted_states(latents, days):
        while tf.rank(days[0]) < tf.rank(latents[0]):
            days = [tf.expand_dims(x, axis=-1) for x in days]
        past = latents[1] + (days[0] - days[2]) / (days[1] - days[2]) * (
            latents[1] - latents[2]
        )
        missing = latents[0] + (days[1] - days[0]) / (days[2] - days[0]) * (
            latents[2] - latents[0]
        )
        future = latents[0] + (days[2] - days[0]) / (days[1] - days[0]) * (
            latents[1] - latents[0]
        )
    ...
    def train_step(imgs, days):
        ...
        with tf.GradientTape() as tape:
            ...
            for image_batch in imgs:
                latent = model.encode(image_batch, training=True)
                latents.append(latent)

            predicted_latents = get_predicted_states(latents, days)
            predicted_imgs = [model.decode(latent) for latent in predicted_latents]
    ...
    def eval_step(imgs, days):
        ...
        latents = []
        for image_batch in imgs:
            latent = model.encode(image_batch, training=False)
            latents.append(latent)
        predicted_latents = get_predicted_states(latents, days)
        predicted_imgs = [model.decode(latent, training=False) for latent in latents]
        if predicted_imgs_for_vis is None:
            predicted_imgs_for_vis = predicted_imgs
        image_similarity_mse = [
            mse_loss_fn(real, pred) for real, pred in zip(imgs, predicted_imgs)

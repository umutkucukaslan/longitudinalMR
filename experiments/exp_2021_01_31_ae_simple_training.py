...
- no latent vector manipulation performed during training 
- no structure similarity vector contraint
- no missing/future scan sampling performed during training
...
if __name__ == "__main__":
    ...
    def train_step(imgs):
        with tf.GradientTape() as tape:
            structure, state = model.encode(imgs, training=True)
            predicted_imgs = model.decode(structure, state)
    ...
    def eval_step(imgs):
        structure, state = model.encode(imgs, training=False)
        predicted_imgs = model.decode(structure, state)
        image_similarity_mse = mse_loss_fn(imgs, predicted_imgs)
        total_loss = tf.reduce_mean(image_similarity_mse)
        ssims = calculate_ssim([imgs], [predicted_imgs])

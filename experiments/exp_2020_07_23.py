...
from model.losses import l2_loss_longitudinal, wgan_gp_loss_longitudinal

"""
Longitudinal training -> Basic autoencoder structure with Adversarial training
Input:  192x160
...
discriminator = gan.get_discriminator(
    input_shape=(INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNEL), batch_norm=False
)
if __name__ == "__main__":
...
if __name__ == "__main__":
    ...
    def train_step(imgs, days, train_generator=True, train_discriminator=True):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            gen_loss, disc_loss, gp_loss = wgan_gp_loss_longitudinal(
                discriminator, imgs, generated_images, LAMBDA_GP
            )
        if train_generator:
        ...
        if train_discriminator:
            ...
            if CLIP_BY_NORM is not None:
                discriminator_gradients = [
                    tf.clip_by_norm(t, CLIP_BY_NORM) for t in discriminator_gradients
                ]
            if CLIP_BY_VALUE is not None:
                discriminator_gradients = [
                    tf.clip_by_value(t, -CLIP_BY_VALUE, CLIP_BY_VALUE)
    ...
    def fit(train_ds, val_ds, train_images, val_images, num_epochs, initial_epoch=0):
        ...
        for epoch in range(initial_epoch, num_epochs):
            ...
            for n, inputs in train_ds.enumerate():
                ...
                losses[4].append(gen_loss.numpy())
                losses[5].append(disc_loss.numpy())
                losses[6].append(gp_loss.numpy())
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar("gen_loss", losses[4], step=epoch)
                tf.summary.scalar("disc_loss", losses[5], step=epoch)
                tf.summary.scalar("gp_loss", losses[6], step=epoch)
            ...
            for inputs in val_ds:
                ...
                val_losses[4].append(gen_loss.numpy())
                val_losses[5].append(disc_loss.numpy())
                val_losses[6].append(gp_loss.numpy())
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar("val_gen_loss", val_losses[4], step=epoch)
                tf.summary.scalar("val_disc_loss", val_losses[5], step=epoch)
                tf.summary.scalar("val_gp_loss", val_losses[6], step=epoch)
            summary_writer.flush()
    ...
    try:
        ...
        log_print("Discriminator train steps: " + str(DISC_TRAIN_STEPS))
        log_print(" ")

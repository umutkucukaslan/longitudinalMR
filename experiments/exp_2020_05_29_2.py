...
LATENT_DISTRIBUTION_STD = 0.5   # normalize latent vector distribution to zero mean gaussian of this std
...
latent_std, latent_mean = encoder(gen_in)  # mean and std for the distribution
s = tf.keras.layers.Multiply()([gen_random, latent_std])
...
if __name__ == "__main__":
    ...
    def train_step(input_image, target, train_generator=True, train_discriminator=True):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            _, _, kl_loss = vae_loss(target, generated_image, latent_mean, latent_std, LATENT_DISTRIBUTION_STD)

            gen_loss, disc_loss, gp_loss = wgan_gp_loss(discriminator, target, generated_image, LAMBDA_GP)
            total_loss = kl_loss + gen_loss
        ...
        return total_loss, gen_loss, kl_loss
    ...
    def fit(train_ds, num_epochs, val_ds, test_ds, train_ds_images, initial_epoch=0):
        ...
        for epoch in range(initial_epoch, num_epochs):
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar('gen_loss', losses[1], step=epoch)
            ...
            for input_image in val_ds:
                total_loss, gen_loss, kl_loss = eval_step(input_image, input_image)
                val_losses[0].append(total_loss.numpy())
                val_losses[1].append(gen_loss.numpy())
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar('val_gen_loss', val_losses[1], step=epoch)

...
Used encoder decoder architecture similar to Feature2Mass paper.
"""
...
# DATASET
train_ds, train_ds2, val_ds, test_ds = get_adni_dataset(machine=MACHINE, return_two_trains=True)
...
def mse_loss(prediction, target):
    mse_loss = tf.reduce_mean(tf.math.square(prediction, target))
...
def train_generator_only_step(input_image, target):
    with tf.GradientTape() as gen_tape:

        gen_output = generator(input_image, training=True)
        gen_mse_loss = mse_loss(gen_output, target)

    generator_gradients = gen_tape.gradient(gen_mse_loss, generator.trainable_variables)
    if CLIP_BY_NORM is not None:
        generator_gradients = [tf.clip_by_norm(t, CLIP_BY_NORM) for t in generator_gradients]
    if CLIP_BY_VALUE is not None:
        generator_gradients = [tf.clip_by_value(t, -CLIP_BY_VALUE, CLIP_BY_VALUE) for t in generator_gradients]
...
def eval_generator_only_step(input_image, target):
    gen_output = generator(input_image, training=False)
    gen_mse_loss = mse_loss(gen_output, target)
    ...
    for epoch in range(initial_epoch, num_epochs):
        ...
        for n, input_image in train_ds.enumerate():
            if TRAIN_ADVERSARIALLY:
                ...
                gen_mse_loss = train_generator_only_step(input_image, input_image)
                losses[0].append(0)
                losses[1].append(0)
                losses[2].append(gen_mse_loss.numpy())
                losses[3].append(0)
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar('gen_mse_loss', losses[2], step=epoch)
        ...
        for input_image in val_ds:
            if TRAIN_ADVERSARIALLY:
                ...
                gen_mse_loss = eval_generator_only_step(input_image, input_image)
                val_losses[0].append(0)
                val_losses[1].append(0)
                val_losses[2].append(gen_mse_loss.numpy())
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar('val_gen_mse_loss', val_losses[2], step=epoch)

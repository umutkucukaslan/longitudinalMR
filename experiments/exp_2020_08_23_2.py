...
"""
Deeper encoder and decoder using basic autoencoder structure with layer normalization.
+ Data augmentation
...
Encoder: filters x ( (num_repeat - 1) x Conv2d + LN + Activation + DepthwiseConv2d + LN + Activation )
Decoder: reverse_filters x ( (num_repeat - 1) x Conv2d + LN + Activation + DepthwiseConv2d + LN + Activation ) + 
            Conv2d + Sigmoid
Latent: 16
Input:  192x160
...
if __name__ == "__main__":
    ...
    def train_step(imgs, days, input_imgs):
        ...
        with tf.GradientTape() as gen_tape:
            a1 = days[1] - days[0]
            a2 = days[2] - days[1]
            latent0 = encoder(input_imgs[0], training=True)
            latent2 = encoder(input_imgs[2], training=True)
    ...
    def eval_step(imgs, days, input_imgs):
        a1 = days[1] - days[0]
        a2 = days[2] - days[1]
        latent0 = encoder(input_imgs[0], training=True)
        latent2 = encoder(input_imgs[2], training=True)
    ...
    def fit(train_ds, val_ds, train_images, val_images, num_epochs, initial_epoch=0):
        ...
        for epoch in range(initial_epoch, num_epochs):
            ...
            for n, inputs in train_ds.enumerate():
                imgs = inputs["imgs"]
                days = inputs["days"]
                input_imgs = inputs["augmented_imgs"]
                (
                    total_reconst_loss,
                    mid_reconst_loss,
                    ssim_total,
                    ssim_missing_reconst,
                ) = train_step(imgs, days, input_imgs=input_imgs)
            ...
            for inputs in val_ds:
                ...
                (
                    ...
                    ssim_total,
                    ssim_missing_reconst,
                ) = eval_step(imgs, days, input_imgs)
